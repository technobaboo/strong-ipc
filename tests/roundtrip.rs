...
async fn payloads_arrive_intact() {
	...
	for len in [1usize, 2, 64, 512, 4096, 8191, 8192] {
		...
		assert!(
			fds.is_empty(),
			"unexpected descriptors on a {len} B payload"
		);
	}
}
...
async fn zero_length_message_looks_like_a_hangup() {
	...
	node_ref.try_send(Message::from_data(Vec::new())).unwrap();
	...
	assert!(
		matches!(
			node_ref.try_send(Message::from_data(b"after".to_vec())),
...
async fn oversized_messages_are_refused_at_the_sender() {
	...
	assert!(
		matches!(
			node_ref
				.send(Message::from_data(vec![
					3u8;
					strong_ipc::MAX_MESSAGE_SIZE + 1
				]))
				.await,
