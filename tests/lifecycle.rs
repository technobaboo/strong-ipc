...
async fn dropping_the_last_ref_still_drains_the_queue() {
	...
	while (seen.lock().unwrap().len() as u32) < accepted && tokio::time::Instant::now() < deadline {
...
async fn the_outbox_can_be_built_from_a_non_runtime_thread() {
	...
	assert!(
		accepted > 0,
		"nothing was accepted from the off-runtime thread"
	);
...
async fn a_ref_notices_its_node_going_away() {
	...
	assert!(
		orphan.is_dead(),
		"is_dead disagrees with death_notification"
	);
