...
async fn parent_main(args: Vec<String>) {
	...
	println!("  frame budget        {:.2} µs", period.as_secs_f64() * 1e6);
	println!("  payload             {} B", cfg.payload);
	println!("  caps per message    {}", cfg.caps);
	println!(
		"  burst mode          {}",
		if cfg.sequential {
			"sequential (await each reply)"
		} else {
			"pipelined (send all, then await)"
		}
	);
	println!(
		"  between frames      {}",
		if cfg.busy_wait {
			"BUSY-WAIT (control run — never parks)"
		} else {
			"sleep (parks the process, like a real app)"
		}
	);
	...
	println!(
		"  transport           {}",
		if cfg.raw {
			"RAW seqpacket (floor — no strong-ipc, caps ignored)"
		} else {
			"strong-ipc"
		}
	);
	...
	let (_reply_node, reply_node_ref) =
		Node::new(ReplyHandler { tx }).expect("failed to build the reply node");
	...
	if msg_ns.is_empty() {
		...
		println!("  budget              {budget_us:.2} µs ({:.0} Hz)", cfg.hz);
		println!(
