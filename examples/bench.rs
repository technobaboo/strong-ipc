...
impl Client {
	...
	async fn quiesce(&mut self) {
		loop {
			...
			let quiet = tokio::time::timeout(Duration::from_millis(200), self.rx.recv())
				.await
				.is_err();
	...
	async fn throughput(
		...
		(sent, self.received.load(Ordering::Relaxed), retries, usage)
	}
}
...
async fn parent_main(args: Vec<String>) {
		...
		.and_then(|s| s.parse::<u64>().ok())
	{
		...
		*watch.timeline.lock().unwrap() = Some((Instant::now(), Vec::new()));
		...
		let mut r = 0;
		client
			.send(Message::from_data(b"QUIT".to_vec()), &mut r)
			.await;
	...
	for (payload, caps) in [
		...
		client.quiesce().await;
		let (sent, recv, retries, u) = client
			.throughput(payload, caps, Duration::from_secs(3))
			.await;
	...
	println!(
		"  MAX_MESSAGE_SIZE is {} B. anything larger is refused at the call site, so",
		MAX_MESSAGE_SIZE
	);
