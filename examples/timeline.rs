...
impl Handler for ReplyHandler {
	async fn handle(&self, data: &mut [u8], _fds: FdVec, _creds: Option<strong_ipc::UCred>) {
		let t3 = now_ns();
		if data.len() >= HDR {
			let _ = self.tx.send((get(data, 0), get(data, 1), get(data, 2), t3));
		}
	}
}
...
async fn parent_main(args: Vec<String>) {
	...
	println!(
		"  between frames      {}",
		if busy_wait {
			"BUSY-WAIT (never parks)"
		} else {
			"sleep (parks, like a real app)"
		}
	);
	println!(
		"  runtime             {}",
		if threads <= 1 {
			"current_thread".to_string()
		} else {
			format!("multi_thread, {threads} workers")
		}
	);
